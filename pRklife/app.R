...
server <- function(input, output, session){
  ...
  genPlaylist <- eventReactive(input$generatePlaylist, {
    ...
    recs <- get_n_closest_songs(song=grab_songs, 
      ...
      unique
    #print(recs)
    generate_playlist(recs, min(20, length(unique(recs$track_name))))
  }
  )
  ...
  observeEvent(input$makeOnSpotify, {
    ...
    if(is.null(user_info$error)){
      spotify_playlist <- spotifyr::create_playlist(user_id=user_info$id,
                                                    name=input$playlistName,
                                                    public = FALSE,
                                                    description = "Created with pRklife app")
      if(!is.null(spotify_playlist$error.status)){
        showNotification(spotify_playlist$error.message %>% as.character())
      }else{
        my_playlist <- genPlaylist()
        spotifyr::add_tracks_to_playlist(playlist_id = spotify_playlist$id,
                                         uris =  my_playlist$track_uri)
